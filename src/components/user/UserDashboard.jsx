    ...
    <div>
      ...
      {days !== null && days <= 7 && (
        <div className={`alert ${days <= 2 ? 'alert-error' : 'alert-warning'}`} style={{ marginBottom: 24 }}>
          ⚠️ <strong>
            Your subscription {days <= 0 ? 'has expired!' : `expires in ${days} day${days !== 1 ? 's' : ''}!`}
          </strong>{' '}
      ...
      <div className="stats-grid" style={{ marginBottom: 24 }}>
        <div className="stat-card green">
          ...
          <div className="stat-sub">
            {member?.nextDueDate
              ? `Due: ${format(new Date(member.nextDueDate), 'dd MMM yyyy')}`
              : 'No due date'}
          </div>
        </div>
        ...
        <div className="stat-card purple">
          ...
          <div className="stat-value">
            {member?.weight ?? '—'}
            <span style={{ fontSize: 16, fontWeight: 400 }}>{member?.weight ? 'kg' : ''}</span>
          </div>
      ...
      {/* Two-column cards — stacks on mobile via CSS class */}
        ...
        <div className="card">
          ...
          {member?.weight && (
            <div style={{
              ...
              <div style={{ fontSize: 36, fontFamily: 'var(--font-display)', color: 'var(--accent-green)' }}>
                {member.weight} <span style={{ fontSize: 18 }}>kg</span>
              </div>
        ...
        <div className="card">
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 0 }}>
            {[
              ...
              <div key={label} style={{
                ...
                <span style={{ fontSize: 13, color: 'var(--text-primary)', maxWidth: '55%', textAlign: 'right', wordBreak: 'break-word' }}>
                  {value}
                </span>
              </div>
            ))}
          ...
          {member?.hasPersonalTrainer && (
            <div style={{
              ...
              <div style={{ fontSize: 11, color: 'var(--trainer-accent)', textTransform: 'uppercase', letterSpacing: 1, marginBottom: 4 }}>
                Personal Trainer
              </div>
              <div style={{ fontWeight: 600, color: 'var(--text-primary)' }}>
                🏋️ {member.personalTrainerName || 'Assigned'}
              </div>
            </div>
          )}
        </div>
